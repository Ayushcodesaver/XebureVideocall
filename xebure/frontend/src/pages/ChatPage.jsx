...
const ChatPage = () => {
  ...
  // ✅ SINGLE Chat initialization (removed duplicate)
  useEffect(() => {
    ...
    const initChat = async () => {
      try {
        ...
        // ✅ Disconnect if somehow already connected
        if (client.user) {
  ...
  useEffect(() => {
    const handleVisibilityChange = () => {
      if (document.visibilityState === 'visible') {
        setTimeout(() => {
          if (chatClient && !chatClient.user && !isReconnecting.current && !isConnecting.current) {
            reconnectChat();
          }
        }, 1000);
      }
            ...
            <div className="p-4">
              <div className="flex items-center gap-3 mb-3">
                <div className={`w-12 h-12 rounded-full flex items-center justify-center ${isVideoCall ? 'bg-green-500/20' : 'bg-blue-500/20'}`}>
                  {isVideoCall ? <Video className="w-6 h-6 text-green-500" /> : <Phone className="w-6 h-6 text-blue-500" />}
                </div>
                <div>
              ...
              <p className="text-xs text-base-content/40 mt-3 text-center">⏰ Link valid for 1 hour</p>
            </div>
          </div>
        </div>
      );
                ...
                <Window className="flex flex-col flex-1 min-h-0">
                  ...
                  <div className="flex-1 overflow-y-auto p-4 space-y-2">
                    {messages.map((message, idx) => (
                      <div key={message.id || idx}>{renderCustomMessage(message)}</div>
                    ))}
                    <div ref={messagesEndRef} />
                  </div>
                  
                  <div className="str-chat__message-input border-t border-base-300 bg-base-100 flex-shrink-0">
                    {showVoiceRecorder && <div className="mb-2"><VoiceRecorder onSend={handleVoiceMessage} onCancel={handleCancelVoice} /></div>}
                    {showFileUploader && <div className="mb-2"><FileUploader onUpload={handleFileUpload} onCancel={handleCancelFileUpload} /></div>}
                    {showGifPicker && <div className="mb-2"><GifPicker onSelect={handleGifSelect} onCancel={handleCancelGifPicker} /></div>}
                    
                    <div className="flex items-center gap-2 bg-base-100 rounded-xl border border-base-300 p-2">
                      <button onClick={() => { setShowFileUploader(!showFileUploader); setShowGifPicker(false); setShowVoiceRecorder(false); }} className="p-2 rounded-full hover:bg-base-200">
                        <Paperclip className="w-5 h-5 text-base-content/60" />
                      </button>
                      <button onClick={() => { setShowGifPicker(!showGifPicker); setShowFileUploader(false); setShowVoiceRecorder(false); }} className="p-2 rounded-full hover:bg-base-200">
