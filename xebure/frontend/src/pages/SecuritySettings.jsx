...
import { 
  Shield, Lock, Fingerprint, Clock, Eye, EyeOff, Ban, Flag, 
  Bell, Smartphone, Key, CheckCircle, AlertTriangle, ChevronRight,
  RefreshCw, Download, Trash2, Globe, Mail, Phone, User, Settings, LogOut
} from "lucide-react";
...
const SecuritySettings = () => {
  ...
  const { rotateEncryptionKey, encryptionKey } = useEncryptionContext();
  ...
  // ============= TEMPORARILY DISABLE FAILING API CALLS ============= 

  // ✅ Blocked users - disabled for now
  useEffect(() => {
    // Backend route not ready yet
    setBlockedUsers([]);
  }, []);
  ...
  // ✅ 2FA status - only when user is authenticated
  useEffect(() => {
    if (!authUser) return;
    ...
    const check2FA = async () => {
      try {
        ...
        if (response.ok) {
          ...
          // Default to false if endpoint not ready
          setIs2FAActive(false);
        }
      } catch (error) {
        console.log("2FA endpoint not ready yet, using default:", error.message);
        setIs2FAActive(false);
      }
    };
    check2FA();
  ...
  const handleUnblock = async (userId) => {
    try {
      await fetch(`/api/users/blocked/${userId}`, { method: 'DELETE', credentials: 'include' });
  ...
  // Return JSX (your existing UI remains the same)
  return (
            ...
            <div className="bg-gray-800/50 backdrop-blur-sm rounded-xl border border-gray-700 overflow-hidden">
              ...
              <div className="p-4">
                ...
                <button 
                  ...
                  className="w-full mt-3 px-3 py-1.5 text-sm bg-gray-700 text-white rounded-lg hover:bg-gray-600 transition-all"
                >
