...
import { User, Mail, Lock, Eye, EyeOff, Sparkles, CheckCircle, AtSign } from "lucide-react";  // ✅ Added AtSign
...
const SignUpPage = () => {
  ...
  const handleSignup = (e) => {
    ...
    if (!usernameRegex.test(signupData.username)) {
      alert("Username must be 3-30 characters and can only contain letters, numbers, underscore, and dot");
      return;
    }
                    ...
                    <div className="form-control w-full space-y-2">
                      <label className="label px-1">
                        <span className="label-text font-medium text-gray-700 flex items-center gap-2">
                          <AtSign className="w-4 h-4 text-[#00A19B]" />
                          Username
                        </span>
                      </label>
                      <input
                        ...
                        onChange={(e) => setSignupData({ ...signupData, username: e.target.value.toLowerCase().replace(/[^a-zA-Z0-9_.]/g, '') })}
                        required
                      />
                      <p className="text-xs text-gray-400 mt-1">
                        Username must be 3-30 characters. Only letters, numbers, underscore(_), and dot(.)
                      </p>
                    </div>
