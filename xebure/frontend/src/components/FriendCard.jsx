...
const FriendCard = ({ friend, index = 0, currentUser }) => {
  ...
  // Calculate real match percentage based on languages and location
  useEffect(() => {
    const calculateMatchPercentage = () => {
      let score = 0;
      let total = 0;

      // Language match (60% weight)
      if (friend.nativeLanguage === currentUser?.learningLanguage) {
        score += 30;
      }
      if (friend.learningLanguage === currentUser?.nativeLanguage) {
        score += 30;
      }
      total += 60;

      // Location match (20% weight) - same city
      if (friend.location && currentUser?.location) {
        const friendCity = friend.location.split(',')[0].trim().toLowerCase();
        const userCity = currentUser.location.split(',')[0].trim().toLowerCase();
        if (friendCity === userCity) {
          score += 20;
        }
      }
      total += 20;

      // Learning same language (10% weight)
      if (friend.learningLanguage === currentUser?.learningLanguage) {
        score += 10;
      }
      total += 10;

      // Native same language (10% weight)
      if (friend.nativeLanguage === currentUser?.nativeLanguage) {
        score += 10;
      }
      total += 10;
  ...
  // Check online status (you'll need backend API for this)
  useEffect(() => {
    const checkOnlineStatus = async () => {
      try {
        // If you have an API to check online status
        const response = await axiosInstance.get(`/users/${friend._id}/status`).catch(() => ({ data: { isOnline: false } }));
  ...
  return (
    ...
    >
      ...
      {/* Online Status Indicator - REAL STATUS */}
      <div className="absolute top-3 left-3 z-20 flex items-center gap-1 bg-base-300/70 backdrop-blur-sm rounded-full px-2 py-0.5">
        <div className={`w-1.5 h-1.5 rounded-full animate-pulse ${isOnline ? 'bg-green-500' : 'bg-gray-400'}`}></div>
        <span className="text-[10px] text-base-content">{isOnline ? 'Online' : 'Offline'}</span>
      </div>
      ...
      <div className="relative z-10 p-5 bg-base-100/95 backdrop-blur-sm">
        ...
        {/* Mutual Interests Tag - REAL MATCH PERCENTAGE */}
        <div className="mt-3 flex items-center justify-between">
          <div className="flex items-center gap-1">
            <Sparkles className="w-3 h-3 text-yellow-400" />
            <span className="text-[10px] text-base-content/50">{matchPercentage}% match</span>
          </div>
          <div className={`text-[10px] flex items-center gap-1 ${isOnline ? 'text-green-500' : 'text-base-content/50'}`}>
            {isOnline ? (
              <>
